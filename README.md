# Проект "Мои банковские операции"

## Описание:

Проект "Мои банковские операции" - это виджет на Python, 
который показывает несколько последних успешных банковских операций клиента.


## Установка:

+ Установите на свой компьютер GitHub.
+ Клонируйте репозиторий с GitHub.
```
 git clone https://github.com/List-8919/Homework.git
```

## Установка зависимости:

Для управления зависимостями часто используется файл requirements.txt. 
Он содержит список библиотек, необходимых для проекта, с указанием их версий.

Создание requirements.txt:
```
pip freeze > requirements.txt
```

Для установки зависимостей, выполните команды:
```
pip install -r requirements.txt
```

## Конфигурационные шаги

+ Файл pyproject.toml. Конфигурационный файл в формате TOML, 
который используется многими инструментами для управления проектами.
+ .gitignore — текстовый файл в системе контроля версий Git, который 
определяет игнорируемые файлы и каталоги.
+ .flake8 — это инструмент для статического анализа кода Python, который проверяет соответствие стилю PEP 8, 
выявляет синтаксические ошибки, нарушения стиля и другие проблемы качества кода.

## Примеры использования функций:

+ Модуль masks
содержит функции маскировки номера банковской карты и маскировки номера банковского счета:

**get_mask_card_number(card_number: str) -> str**

Принимает на вход номер карты в виде числа и возвращает маску номера

     Пример: 7000792289606361 → 7000 79** **** 6361

**get_mask_account(account_number: int | str) -> str**

Принимает на вход номер счета и возвращает маску номера 

     Пример: 73654108430135874305 → **4305

+ Модуль widget
содержит функции для работы с банковскими данными:

**mask_account_card(account_card: str) -> str**

Принимает один аргумент — строку, содержащую тип и номер карты или счета, 
и возвращает строку с замаскированным номером.

     Пример: "Visa Platinum 7000792289606361" → "Visa Platinum  7000 79** **** 6361"

**get_date(date_format: str) -> str**

Принимает на вход строку с датой в формате "2024-03-11T02:26:18.671407"
и возвращает строку с датой в формате "ДД.ММ.ГГГГ" ("11.03.2024").

      Пример: "2024-03-11T02:26:18.671407" → "11.03.2024"

+ Модуль processing
содержит  функции для фильтрации и сортировки банковских операций:

**filter_by_state(data: list, state: str = 'EXECUTED') -> list:**

Фильтрует список операций по статусу выполнения (например по 'EXECUTED')

      Пример: [{'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
               {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'},
               {'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'},
               {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'}] 

         →   →   [{'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
                  {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'}]
    

**sort_by_date(items, descending=True)**

Сортирует список операций по дате (например по убыванию)

      Пример: [{'id': 414288290, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
                  {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'},
                  {'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'},
                  {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'}]

        →   →   [{'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
                 {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'}]


## Автор проекта:

KAA